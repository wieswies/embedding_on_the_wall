...
def train_dev_dimensions(pipeline, X_data, y_data, thresholds):
    ...
    '''
    results = []
    html_report = f'<html><head><title style="color:black;font-family:Palatino;text-align:center;"> Model Evaluation Report for {pipeline}</title></head><body>'
    ...
    with open(f"Evaluation_report_for_{pipeline}.html", "w") as f:
        f.write(html_report)
...
def test_dimensions(pipeline, X_data, y_data, thresholds):
    ...
    '''
    results = []
    html_report = f'<html><head><title style="color:black;font-family:Palatino;text-align:center;"> Model Test Report for {pipeline}</title></head><body>'
    ...
    # Save the report as an HTML file
    with open(f"Test_report_for_{pipeline}.html", "w") as f:
        f.write(html_report)
